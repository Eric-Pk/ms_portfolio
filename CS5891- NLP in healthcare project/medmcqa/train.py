...
def train(gpu,
    ...
    inference_model = inference_model.to("gpu")
    ...
    test_df.loc[:, "predictions"] = [pred + 1 for pred in
                                     run_inference(inference_model, mcqaModel.test_dataloader(), args)]
    ...
    val_df.loc[:, "predictions"] = [pred + 1 for pred in
                                    run_inference(inference_model, mcqaModel.val_dataloader(), args)]
...
if __name__ == "__main__":
    ...
    #     exit()
    print(
        f"Training started for model - {model} variant - {exp_dataset_folder} use_context - {str(cmd_args.use_context)}")
    ...
    exp_name = f"{model}@@@{os.path.basename(exp_dataset_folder)}@@@use_context{str(cmd_args.use_context)}@@@seqlen{str(args.max_len)}".replace(
        "/", "_")
